...
public class SpringBoot2JdbcWithH2Application implements CommandLineRunner {
	    ...
	    public void run(String...args) throws Exception {
			logger.info("estudiante id 101 -> {}", repository.findById(101));
			logger.info("all users 1 --> {}", repository.findAll());
			logger.info("inserting -> {}",
					repository.insert(new Estudiante(105, "Alfredo Guevara", "comandante@antiimperialista.es")));

			logger.info("update 101 -> {}",
					repository.update(new Estudiante(101, "Clark Ken", "Yonosoysuperman@periodico.es")));

			repository.deleteById(102);

			logger.info("all users 4 -> {}", repository.findAll());
	    }
	}
